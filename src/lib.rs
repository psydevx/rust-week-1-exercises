...
pub fn generate_address(prefix: &str) -> String {
    ...
    let suffix_len = if prefix.len() < 32 { 32 - prefix.len() } else { 0 };
    
    let mut suffix = String::new();
    let mut hash_val = hash;
    for _ in 0..suffix_len {
        let idx = (hash_val as usize) % charset.len();
        suffix.push(charset.chars().nth(idx).unwrap());
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    if height < 0 {
        ...
        (false, "Block height exceeds unrealistic range".to_string())
    } else {
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    // Bitcoin transaction version is the first 4 bytes (8 hex chars)
    if raw_tx_hex.len() < 8 {
        return Err("Transaction data too short".to_string());
    }
    
    let version_hex = &raw_tx_hex[0..8];
    // Bitcoin uses little-endian, so we need to reverse the bytes
